...
namespace v2rayN.Forms
{
    public partial class MainForm : BaseForm
    {
        ...
        public MainForm()
        {
            ...
            HideForm();
            Text = Utils.GetVersion() + Utils.GetExtra();
        ...
        private void RestoreUI()
        {
            ...
            {
                var width = ConfigHandler.GetformMainLvColWidth(ref config, ((EServerColName) k).ToString(),
        ...
        private void StorageUI()
        {
            ...
            for (int k = 0; k < lvServers.Columns.Count; k++)
            {
                ConfigHandler.AddformMainLvColWidth(ref config, ((EServerColName) k).ToString(),
                    lvServers.Columns[k].Width);
            }
        }
        ...
        private void Speedtest(string actionType)
        {
            ...
            ClearTestResult();
            SpeedtestHandler statistics = new SpeedtestHandler(ref config, ref v2rayHandler, lvSelecteds, actionType,
        ...
        {
            SpeedtestHandler statistics =
                new SpeedtestHandler(ref config, ref v2rayHandler, lvSelecteds, "", UpdateSpeedtestHandler);
        ...
        private void menuAddCustomServerUrl_Click(object sender, EventArgs e)
        {
            UI.Show(UIRes.I18N("CustomServerTips"));
            var frm = new AddCustomServerUrlForm {EditIndex = -1};
            frm.ShowDialog();
            RefreshServers();
            LoadV2ray();
        }
        ...
        {
            lvServers.Invoke((MethodInvoker) delegate { SetTestResult(index, msg); });
        }
            ...
            if (downloadHandle == null)
            {
                ...
                downloadHandle.AbsoluteCompleted += (sender2, args) =>
                {
                    if (args.Success)
                    {
                        ...
                        string url = args.Msg;
                        this.Invoke((MethodInvoker) (delegate { askToDownload(downloadHandle, url); }));
                    }
                    else
                    {
                ...
                downloadHandle.Error += (sender2, args) => { AppendText(true, args.GetException().Message); };
            }
        ...
        private void tsbCheckUpdatePACList_Click(object sender, EventArgs e)
        {
            ...
            if (pacListHandle == null)
            {
                ...
                pacListHandle.Error += (sender2, args) => { AppendText(true, args.GetException().Message); };
            }
        ...
        private void tsbCheckClearPACList_Click(object sender, EventArgs e)
        {
            try
            {
                File.WriteAllText(Utils.GetPath(Global.pacFILE), Utils.GetEmbedText(Global.BlankPacFileName),
                    Encoding.UTF8);
        ...
        private void UpdateSubscriptionProcess()
        {
            ...
            for (int k = 1; k <= config.subItem.Count; k++)
            {
                ...
                downloadHandle3.Error += (sender2, args) => { AppendText(true, args.GetException().Message); };
        ...
        private void tsbUpdateConfig_Click(object sender, EventArgs e)
        {
            new Thread(() =>
            {
                var updateVs = config.vmess.Where(v => v.configType == 2 && v.url != "").ToList();
                if (updateVs.Count <= 0)
                {
                    return;
                }

                foreach (var v in updateVs)
                {
                    try
                    {
                        if (!Utils.UrlIsExists(v.url)) continue;
                        var configContent = Utils.GetRemoteCustomConfig(v.url);
                        File.WriteAllText(v.address, configContent);
                    }
                    catch (Exception ex)
                    {
                    }
